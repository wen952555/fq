					...
					if (url.searchParams.get('token') === 订阅TOKEN || 作为优选订阅生成器) {
						...
						if (订阅类型 === 'mixed') {
							...
							try {
								...
								return new Response('订阅转换后端��常：' + error.message, { status: 403 });
							}
						}
...
function 批量替换域名(内容, hosts, 订阅类型 = 'mixed') {
	if (!hosts || hosts.length === 0) return 内容;
	const 打乱后HOSTS = [...hosts].sort(() => Math.random() - 0.5);
	const 字符集 = 'abcdefghijklmnopqrstuvwxyz0123456789';
	
	const getRandomHost = () => {
		const 原始host = 打乱后HOSTS[Math.floor(Math.random() * 打乱后HOSTS.length)];
		return 原始host?.includes('*') ? 原始host.replace(/\*/g, () => {
			let s = '';
	...
	if (订阅类型 === 'mixed' || 订阅类型 === 'vmess') {
		return 内容.split('\n').map(line => {
			const randomHost = getRandomHost();
			return line.replace(/example\.com/g, randomHost);
		}).join('\n');
	} else {
		let count = 0;
		...
		return 内容.replace(/example\.com/g, () => {
			if (count % 2 === 0) {
				currentRandomHost = getRandomHost();
			}
			count++;
